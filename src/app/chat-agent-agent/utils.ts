...
type InferToolArgs<T> = T extends { inputSchema: infer S } ? (S extends z.ZodType ? z.infer<S> : never) : never
...
export function hasToolConfirmation(message: UIMessage): boolean {
  return message?.parts?.some((part) => part.type?.startsWith('tool-') && 'output' in part) || false
}
...
>(
      ...
      if (isToolConfirmationPart(part) && part.type.startsWith('tool-')) {
        ...
        if (output === APPROVAL.YES) {
          ...
          if (toolInstance) {
            ...
            result = await (toolInstance as (args: typeof toolInput) => Promise<string>)(toolInput)
  ...
  return [...messages.slice(0, -1), { ...lastMessage, parts: processedParts.filter(Boolean) }]
}
